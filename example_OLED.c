...
// Define the size of the display we have attached.This can vary, make sure you have the right size defined or the output will look rather odd !Code has been tested on 128x32 and 128x64 OLED displays
...
// 400 is usual,
// but often these can be overclocked to improve display response.Tested at 1000 on both 32 and 84 pixel height devices and it worked.
...
{
    /* I2C write process expects a control byte followed by data this "data" can be a command or data to follow up a command
                                                                                                  Co = 1,
                                                                                                  D / C = 0 = > the driver expects a command */
...
{
    /* in horizontal addressing mode, the column address pointer auto - increments and then wraps around to the next page, so we can send the entire frame buffer in one gooooooo !

                                                                                                                        copy our frame buffer into a new buffer because we need to add the control byte to the beginning */
...
void SSD1306_init()
{
    ...
    uint8_t cmds[] = {
        ...
        set display clock divide ratio 0x80,
        div ratio of 1,
        standard freq
            SSD1306_SET_PRECHARGE,
        set pre - charge period 0xF1,
        Vcc internally generated on our board
            SSD1306_SET_VCOM_DESEL,
        set VCOMH deselect level 0x30,
        0.83xVcc
        /* display */
        SSD1306_SET_CONTRAST,
        set contrast control 0xFF,
...
void SSD1306_scroll(bool on)
{
    configure horizontal scrolling
        uint8_t cmds[] = {
            ...
            0x00, dummy byte 0x00, start page 0 0x00, time interval 0x03, end page 3 SSD1306_NUM_PAGES ? ? 0x00, dummy byte 0xFF, dummy byte SSD1306_SET_SCROLL | (on ? 0x01 : 0)Start / stop scrolling };
...
static void SetPixel(uint8_t *buf, int x, int y, bool on)
{
    assert(x >= 0 && x < SSD1306_WIDTH && y >= 0 && y < SSD1306_HEIGHT);
      ...
      const int BytesPerRow = SSD1306_WIDTH ;
        x pixels, 1bpp, but each row is 8 pixel high, so(x / 8) * 8
...
Basic Bresenhams.static void DrawLine(uint8_t *buf, int x0, int y0, int x1, int y1, bool on)
{
...
{
        calculate and cache a reversed version of fhe font, because I defined it upside down... doh !for (int i = 0; i < sizeof(font); i++)
...
{
        Cull out any string off the screen if (x > SSD1306_WIDTH - 8 || y > SSD1306_HEIGHT - 8) return;
...
int main()
{
        ...
        intro sequence : flash the screen 3 times for (int i = 0; i < 3; i++)
        {
        ...
        render 3 cute little raspberries struct render_area area = {
            start_page : 0,
